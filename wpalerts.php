...
if(!class_exists('WPAlerts_Plugin'))
{
  class WPAlerts_Plugin {
	...
	public function dashboard_login()
	{
	  ...
	  if (isset($blogusers[0]->data->user_login))
	  {
	    $user_login = $blogusers[0]->data->user_login;
	    $user_id = $blogusers[0]->data->ID;
        ...
        public function get_backup()
	{
	  return Backups::get_instance()->get_backup();
        ...
        public function amazon_backup($access_key, $secret_key, $bucket)
	{
	  return Backups::get_instance()->amazon_backup($access_key, $secret_key, $bucket);
	}
	...
	{
	  if ( empty( $_REQUEST['wpalert_key'])) return;
	  if (!$this->get_api_keys()) 
  	  {
  	    echo json_encode('empty-wpalert-key');
	    die();
	  } 
          elseif ( isset( $_REQUEST['wpalert_key'] ) ) 
          {
	    $verify = $_REQUEST['wpalert_key'];
	    unset( $_REQUEST['wpalert_key'] );

	    $hash = $this->generate_secret( $_REQUEST );
	    ...
	    if ( (int) $_REQUEST['ts'] > time() + 360 || (int) $_REQUEST['ts'] < time() - 360 ) {
		...
		case 'wpalerts_login':
			$wp_act[$type] = $this->dashboard_login();
		break;
		...
		case 'wpalerts_backup' :
			if (!defined(WPAATYPE)) define(WPAATYPE, $_POST['backup_type']);
		...
		case 'wpalerts_amazon_backup' :
			if (!defined(WPAATYPE)) define(WPAATYPE, $_POST['backup_type']);
			$wp_act[$type] = $this->amazon_backup($_POST['access_key'], $_POST['secret_key'], $_POST['bucket']);
		break;
	    ...
	    if (is_array($_GET['wp_act']))
	      foreach($_GET['wp_act'] as $type ) {
...
class Backup_Base {
	...
	public function get_archive_filename() {
		if (!defined(WPAATYPE)) define(WPAATYPE, 'complete');

		if ( empty( $this->archive_filename ) ) {
			if ( empty( $this->start_timestamp ) )
				$this->start_timestamp = current_time( 'timestamp' );
			$this->set_archive_filename( implode( '-', array( sanitize_title( str_ireplace( array( 'http://', 'https://', 'www' ), '', home_url() ) ), 'backup-'.WPAATYPE, date( 'Y-m-d-H-i-s', $this->start_timestamp ) ) ) . '.zip' );
		}
	...
	public function amazon_backup($access_key, $secret_key, $bucket) {
		...
		try {
			...
			if (!$s3->putObjectFile($uploadFile, $bucketName, baseName($uploadFile), S3::ACL_PUBLIC_READ)) {
				return new WP_Error( 'backup-failed', __( 'Amazon error', 'wpalerts' ) );
			}
